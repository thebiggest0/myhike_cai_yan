...
function saveHikeDocumentIDAndRedirect() {
    // console.log('test saveHikeDocumentIDAndRedirect')
    let params = new URL(window.location.href); //get URL of search bar
    let ID = params.searchParams.get("docID"); //get value for key "id" search for it
    console.log(ID); // should get result as url

    localStorage.setItem('hikeID', ID)  // can use any key, something that makes sense and can remember
    window.location.href = "review.html"  // redirect to hikeReview.html, passes above info to review page (way of passing info between pages)
}


function populateReviews() {
    console.log("test");
    let hikeCardTemplate = document.getElementById("reviewCardTemplate");  // get hike card template and populate it
