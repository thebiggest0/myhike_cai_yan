var hikeID = localStorage.getItem('hikeID'); // get hikeID from local storage
// console.log(hikeID)
...
function displayHikeName() {
    ...
    db.collection("hikes").doc(hikeID).get().then((thisHike)=>{
        ...
        document.getElementById("hikeName").innerHTML = hikeName;  // this element and we display the hikeName over it
    })
}
...
function writeReview() {
    console.log("inside write review");  // checker
    let hikeTitle = document.getElementById("title").value;     // get content and set it into a variable
    ...
    let hikeFlooded = document.querySelector('input[name="flooded"]:checked').value;        // radio buttons: using query selector check for name 'flooded' and see which one is checked 
    ...
    if (user) {
        ...
        // Get the document for the current user. and starting a new collection called reviews
